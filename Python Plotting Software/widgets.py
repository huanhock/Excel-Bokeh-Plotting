...
from bokeh.models import Callback, Circle, ColumnDataSource, HoverTool, GlyphRenderer
...
cu1_avg = ColumnDataSource(data=dict(y=[cu1_stats['avg'], cu1_stats['avg']]))
...
m_s.callback = Callback(args=dict(m_avg=m_avg, m_lower=m_lower, m_upper=m_upper), code="""
        ...
        m_lower.get('data')['y'] = [ym - (2 * std), ym - (2 * std)]
        m_upper.get('data')['y'] = [(2 * std) + ym, (2 * std) + ym]
